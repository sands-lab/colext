...
class FlowerClient<X : Any, Y : Any>(
    ...
    init {
        try {
            ...
            this.interpreter = Interpreter(tfliteFileBuffer, options)
        }
        catch (e:Exception){
            Log.d(TAG, e.stackTraceToString())
            this.interpreter = Interpreter(tfliteFileBuffer)
        }
    }
     ...
     */
    fun fit(
        epochs: Int = 1, batchSize: Int = 32, cirID: Int, lossCallback: ((List<Float>) -> Unit)? = null
    ): List<Double> {
    ...
    private fun trainOneEpoch(batchSize: Int): List<Float> {
        ...
        return trainingBatches(min(batchSize, trainingSamples.size)).map {
            ...
            count++
            training(spec.convertX(bottlenecks), spec.convertY(labels), count)
        }.toList()
    }
    ...
    private fun training(
        bottlenecks: Array<X>, labels: Array<Y>, count: Int
    ): Float {
        ...
        val tEnd = Timestamp(System.currentTimeMillis())
        loggingService?.writeBatch(tStart, tEnd, loss.get(0), count, tStart, tStart, tStart)
        Log.d("Batch", "END")
        return loss.get(0)
    }
