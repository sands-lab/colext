...
def MonitorFlwrStrategy(FlwrStrategy):
    ...
    class MonitorFlwrStrategy(FlwrStrategy):
        def __init__(self, *args, **kwargs):
            ...
            self.JOB_ID = os.getenv("FLTB_JOB_ID")
            if self.JOB_ID == None:
                print(f"Tried to use FL testbed but FLTB_JOB_ID env variable is not defined. Exiting.") 
                exit(1)
        ...
        def get_client_db_id(self, client_proxy: ClientProxy) -> int:
            ...
            if ip_address not in self.clients_ip_to_id:
                ...
                properties_res: GetPropertiesRes = client_proxy.get_properties(GetPropertiesIns(config={"FLTB_CLIENT_DB_ID": "?"}), timeout=3)
                assert properties_res.status.code == Code.OK
                client_id = properties_res.properties["FLTB_CLIENT_DB_ID"]
